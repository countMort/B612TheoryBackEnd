...
async function createThumbnail() {
    ...
    for (let index = 0 ; index < products.length; index++) {
        ...
        const thumbnailPath = photoPath.slice(length, formatLength) + "-thumbnail.png"
        ...
        await sharp(tempThumbnailPath).resize({width: 80, height: 80, fit: 'inside'}).png().toFile(tempThumbnailPath + '.png')
...
async function sendSharpedPhotos() {
    try {
        setTimeout(async () => {
            ...
            await download('https://dl.b612theory.ir/admin/' + fileName, tempThumbnailPath)
            await sharp(tempThumbnailPath).resize({width: 100, height: 100, fit: 'inside'}).png().toFile('thumbnail'+tempThumbnailPath)
            const url = await putFile('thumbnail'+tempThumbnailPath, '/admin/' + 'thumbnail-' + fileName)
            console.log(url);
        }, 2000)
    } catch (error) {
        console.log(error);
...
async function testDownloadSpeed() {
    for (let index = 0; index < 20; index++) {
        ...
        await download('https://dl.b612theory.ir/admin/kahkeshan.jpg', `./routes/temp files/${index}.png`)
