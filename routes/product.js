...
router.get('/admin/products', async (req, res) => {
  try {
    let result
    if (req.query.name) {
      result = await Product.findOne({ name: req.query.name })
...
router.get('/products', async (req, res) => {
  try {
    let result
    if (req.query.name) {
      result = await Product.findOne({ name: req.query.name }, { types: 0 })
        .populate('category', 'name value')
        .populate('reviews', 'rating')
        .exec()
      if (result) return responses(res, 200, ['محصول'], result)
    } else {
      result = await Product.find({}, { types: 0 })
        .populate('category', 'name value')
        .populate('reviews', 'rating')
        .exec()
      if (result) return responses(res, 200, ['محصولات'], result)
...
router.get('/products/:id', async (req, res) => {
  try {
    let product = await Product.findOne({ _id: req.params.id })
      ...
      .exec()
    if (!product)
      product = await Product.findOne({ types: req.params.id })
        .populate('category types')
        .populate('reviews', 'rating')
        .exec()
    ...
    return responses(res, 200, ['محصولات', 'نوع ها'], {
      product,
      type: product.types.find((type) => type._id == req.params.id),
    })
  } catch (error) {
  ...
  try {
    ...
    if (types) {
      ...
      product.types = product.types.filter((productType) => {
        return types.find(
          (type) => JSON.stringify(type._id) == JSON.stringify(productType)
        )
      })
      // console.log('product.types: ', product.types);
      for (let index = 0; index < types.length; index++) {
