...
router.post("/order",verifyToken, async (req,res)=> {
    try {
        ...
        }).populate('product product.category').exec()
        ...
        cart.forEach(cartItem => {
            foundTypes.forEach(type => {
                if (cartItem._id == type._id) {
                    ...
                    type.product.stockQuantity = type.product.stockQuantity - cartItem.quantity
        ...
        for (let index = 0; index < foundTypes.length; index++) {
            ...
            console.log(foundTypes[index].product.name, foundTypes[index].product.quantity);
        }
        return res.json({
            success : true,
...
router.get("/orders", verifyToken, async (req, res) => {
    try {
        let orders = await Order.find({ user : req.decoded._id}).populate("products.category products.product").exec()
        return res.json({
            success : true,
            orders
        })
...
router.get('/orders/polaroids/:id', [verifyToken, isAdmin], async (req, res) => {
    try {
        const order = await Order.findOne({_id: req.params.id}).populate('products.polaroid').exec()
...
router.get("/admin/orders", [verifyToken,isAdmin], async (req, res) => {
    try {
        ...
        if(req.query.status && req.query.status == 'posted') query = {...query, createdTime: {$gte : Date.now() - 35*1000*60*60*24}}
        console.log(query);
        let orders = await Order.find(query).populate("products.category products.product products.voiceCard products.polaroid products.diaryBook").sort([['paymentDate', -1]]).exec()
...
router.put('/order/:id', verifyToken, async (req,res) => {
    try {
        ...
        let order = await Order.findOne({_id : req.params.id}).populate('user products.voiceCard').exec()
        if(order) {
            ...
            if(isAdmin || JSON.stringify(order.user._id) == JSON.stringify(req.decoded._id)) {
                ...
                if (isAdmin) {
                    if (reqStatus) {
                        switch (reqStatus) {
                            case 'creating':
                                await SMS('creating',order.deliverTo.phoneNumber,{name: order.deliverTo.fullName, trackingCode: order.trackingCode, days: SHIPMENT.normal.days})
                                break;
                            case 'posted':
                                await SMS('posted',order.deliverTo.phoneNumber,{name: order.deliverTo.fullName, link: `b612theory.ir?redirect=post&code=${order.postCode}`})
                                break;
                            case 'reject':
                                await SMS('reject',order.user.phone,{name: order.user.name, trackingCode: order.trackingCode})
                                break;
                            default:
                                break;
                        }
...
router.delete('/order/:id', verifyToken, async (req, res) => {
    try {
        let deletingOrder = await Order.findOneAndDelete({_id: req.params.id}).populate('products.product products.type products.voiceCard products.diaryBook products.polaroid').exec()
        if (req.decoded.role != 'admin' && req.decoded._id != deletingOrder.user) return responses(res,403)
        let box = {
            types: [],
            products: [],
        }
