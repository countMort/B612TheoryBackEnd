...
router.post(
  '/voice-card',
  [voiceCardController, verifyToken],
  async (req, res) => {
    try {
...
router.post(
  '/voice-card/plenty',
  [voiceCardController, verifyToken, isAdmin],
  async (req, res) => {
    try {
...
router.get('/voice-card/:id', decodeToken, async (req, res) => {
  try {
    ...
    if (!voiceCard) return responses(res, 404, ['کارت صدا وجود ندارد'])
      ...
      (req.decoded && ['admin', 'super_admin'].includes(req.decoded.role)) ||
      voiceCard.activated ||
      voiceCard.comparePassword(req.query.password)
    ) {
...
router.put('/voice-card/plenty/:id', [decodeToken], async (req, res) => {
  try {
    ...
    if (
      (req.decoded && ['admin', 'super_admin'].includes(req.decoded.role)) ||
      voiceCard.comparePassword(fields.password)
    ) {
...
router.delete('/voice-card/:id', verifyToken, async (req, res) => {
  try {
    ...
    if (
      ...
      JSON.stringify(voiceCard.user) == JSON.stringify(req.decoded._id)
    ) {
