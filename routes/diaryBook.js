...
router.post('/diary-book', verifyToken , async(req , res) => {
    try {
...
router.delete("/diary-book/:id", verifyToken , async (req , res) => {
    try {
        ...
        if(['admin','super_admin'].indexOf(req.decoded.role) != -1 || JSON.stringify(diaryBook.user) == JSON.stringify(req.decoded._id)) {
