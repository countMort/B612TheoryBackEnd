async function createVoiceCard(req, VoiceCard) {
  ...
  voiceCard.password =
    (req.decoded.role == "admin" && req.body.password) || "000000"
...
async function deleteVoiceCard(voiceCard) {
  ...
  if (voiceCard.photo && voiceCard.photo.indexOf("/admin/") == -1)
    await unload(voiceCard.photo)
  if (voiceCard.thumbnail && voiceCard.thumbnail.indexOf("/admin/") == -1)
    await unload(voiceCard.thumbnail)
